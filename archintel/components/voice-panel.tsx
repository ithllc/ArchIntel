...
export function VoicePanel({ diagramFile, onPipelineTrigger, pipelineStatus, threatResults, costResults }: VoicePanelProps) {
  ...
  const startSession = async () => {
    ...
    try {
      // Parallel: fetch token + request mic + describe diagram via Gemini 2.5 Flash vision
      const diagramDescPromise = diagramFile
        ? fetch('/api/describe-diagram', {
            method: 'POST',
            body: (() => { const fd = new FormData(); fd.append('diagram', diagramFile); return fd; })(),
          }).then(r => r.json()).then(d => d.description as string).catch(() => null)
        : Promise.resolve(null);

      const [tokenData, micStream, diagramDescription] = await Promise.all([
...
When citing specific numbers or findings, say "according to our analysis" to indicate the data is from the automated pipeline.`
      ...
      ws.onmessage = async (event) => {
        ...
        if (msg.setupComplete) {
          ...
          // Send diagram description as text (generated by Gemini 2.5 Flash vision)
          if (diagramDescription) {
            ws.send(JSON.stringify({
              clientContent: {
                turns: [{
                  role: 'user',
                  parts: [{ text: `Here is a detailed description of the architecture diagram I uploaded:\n\n${diagramDescription}\n\nPlease give me a brief overview of this architecture, highlighting the key components and any initial security or cost concerns you notice. I have also kicked off automated security and cost analyses that will provide detailed findings shortly.` }]
                }],
                turnComplete: true
              }
            }));
